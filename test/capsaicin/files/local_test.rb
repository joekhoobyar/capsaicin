...
class Capsaicin::Files::LocalTest < Test::Unit::TestCase
  ...
  def test_mkdir
    assert @local.mkdir(tmpdir('test-mkdir'))
    assert File.directory?(tmpdir('test-mkdir'))
  end
  ...
  def test_mkdir_not_p
    ...
    assert ! File.directory?(tmpdir('test-mkdir','p'))
  end

  def test_mkdir_p
    assert @local.mkdir_p(tmpdir('test-mkdir','p'))
    assert File.directory?(tmpdir('test-mkdir','p'))
  end
